...
proper exclusion of runtime/mlflow/storage/hydra/pipeline keys.
"""
...
class TestCfgHash:
    ...
    def test_pipeline_excluded(self):
        """pipeline (anchors config) should NOT change training cfg_hash."""
        c1 = _make_cfg()
        c2 = OmegaConf.merge(
            _make_cfg(),
            OmegaConf.create({"pipeline": {"anchors": {"per_class": 200}}}),
        )
        assert cfg_hash(c1) == cfg_hash(c2)

    def test_all_excluded_keys_together(self):
        """All five excluded groups present → same hash as base."""
        c1 = _make_cfg()
        c2 = OmegaConf.merge(
            _make_cfg(),
            OmegaConf.create({
