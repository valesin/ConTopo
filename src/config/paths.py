"""
Centralized path resolution utilities.

All output paths are relative to ``cfg.runtime.outputs_root``.
This module provides helper functions to resolve absolute paths
for models, cache, analysis artifacts, etc.

Usage::

    from src.config.paths import get_models_dir, get_cache_dir, get_analysis_dir

    models_dir = get_models_dir(cfg)
    cache_dir = get_cache_dir(cfg)
"""

from __future__ import annotations

from pathlib import Path
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from omegaconf import DictConfig


def get_outputs_root(cfg: DictConfig) -> Path:
    """Return the absolute path to the outputs root directory."""
    return Path(cfg.runtime.outputs_root).resolve()


def get_models_dir(cfg: DictConfig) -> Path:
    """Return the absolute path to the models directory."""
    return get_outputs_root(cfg) / cfg.runtime.paths.models


def get_cache_dir(cfg: DictConfig) -> Path:
    """Return the absolute path to the cache directory (inference, anchors, manifests)."""
    return get_outputs_root(cfg) / cfg.runtime.paths.inference_cache


def get_analysis_dir(cfg: DictConfig) -> Path:
    """Return the absolute path to the analysis directory (diversity, consistency)."""
    return get_outputs_root(cfg) / cfg.runtime.paths.analysis


def get_mlflow_db_path(cfg: DictConfig) -> Path:
    """Return the absolute path to the MLflow SQLite database."""
    # Parse from tracking_uri which is like "sqlite:///outputs/mlflow.db"
    uri = cfg.mlflow.tracking_uri
    if uri.startswith("sqlite:///"):
        db_path = uri[len("sqlite:///") :]
        return Path(db_path).resolve()
    # For non-sqlite URIs, return None or raise
    return get_outputs_root(cfg) / "mlflow.db"


def ensure_output_dirs(cfg: DictConfig) -> None:
    """Create all output directories if they don't exist."""
    dirs = [
        get_outputs_root(cfg),
        get_models_dir(cfg),
        get_cache_dir(cfg),
        get_analysis_dir(cfg),
    ]
    for d in dirs:
        d.mkdir(parents=True, exist_ok=True)


# ─────────── Legacy compatibility aliases ───────────
# These are provided temporarily to ease migration


def get_artifacts_root(cfg: DictConfig) -> Path:
    """Legacy alias for get_cache_dir. Prefer get_cache_dir for new code."""
    return get_cache_dir(cfg)


def get_models_root(cfg: DictConfig) -> Path:
    """Legacy alias for get_models_dir. Prefer get_models_dir for new code."""
    return get_models_dir(cfg)
