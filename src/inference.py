...
def run_model_inference(
    ...
    Returns dict with: preds, labels, logits, probs, embeddings, accuracy.
    """
    encoder.eval()
    ...
    for batch in loader:
        ...
        if classifier is not None:
            ...
            logits = feats  # model already returns logits
        ...
        emb_list.append(feats.detach().cpu())
        offset += bs
...
# ───────────── run inference on a full model (encoder+classifier combined) ─────────────
