...
# --- THE CRITICAL FIX: Import the single source of truth for your split ---
from src.data.loaders import _split_train_val_indices
...
class DatasetManifest:
    ...
    hashes: list[str]  # hex content hashes
    ...
    def manifest_hash(self) -> str:
        """Deterministic hash of manifest content (all hashes + labels)."""
        content = "|".join(self.hashes) + "|" + self.split
            ...
            mlflow.log_params(
                {
                    ...
                    "num_examples": len(manifest.hashes),
...
def get_or_create_manifest(
    ...
    manifest_dir = os.path.join(
        artifacts_root, "dataset_manifests", dataset_name, split
    )
    ...
    if dataset_name != "cifar10":
        raise NotImplementedError(
            f"Manifest generation for '{dataset_name}' is not implemented."
        )

    if split in ["train", "val"]:
        # 1. Ask loaders.py for the exact indices it will use
        train_idx, val_idx = _split_train_val_indices(data_root, val_per_class)

        # 2. Build the master list once
        full_train = _build_cifar10_manifest(data_root, train=True, split_name="train")

        # 3. Apply the exact same masks to the manifest
        if split == "val":
            manifest = DatasetManifest(
                hashes=[full_train.hashes[i] for i in val_idx],
                original_indices=full_train.original_indices[val_idx],
                labels=full_train.labels[val_idx],
                dataset_name="cifar10",
                split="val",
            )
        else:  # split == "train"
            manifest = DatasetManifest(
                hashes=[full_train.hashes[i] for i in train_idx],
                original_indices=full_train.original_indices[train_idx],
                labels=full_train.labels[train_idx],
                dataset_name="cifar10",
                split="train",
            )
    else:
