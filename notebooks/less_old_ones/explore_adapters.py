...
for ens_info in list_ensembles():
    ...
    if os.path.exists(metrics_path):
        ...
        if all(
            x is not None
            for x in [acc_embed, acc_embed_sim, acc_linear_embed, acc_linear_embed_sim]
        ):
...
plt.plot(
    ...
    linewidth=2.5,
    color="dodgerblue",
    label="MLP (Embed Only)",
)

plt.plot(
    x_positions,
    acc_linear_embed_sims,
    marker="s",
    markersize=8,
    linestyle="-",
    linewidth=2.5,
    color="darkred",
    label="Linear (Embed+Sim)",
)
plt.plot(
    x_positions,
    acc_embed_sims,
...
print(
    f"{'Ensemble':<15} | {'Rho':<8} | {'Comp Mean':<10} | {'Ens Soft':<10} | {'Lin Embed':<10} | {'MLP Embed':<10} | {'Lin Emb+Sim':<12} | {'MLP Emb+Sim':<12}"
)
print("-" * 110)
for r in results:
    print(
        f"{r['name']:<15} | {r['rho']:<8.3f} | {r['comp_acc']:<10.4f} | {r['ensemble_acc']:<10.4f} | "
...
print("  \\centering")
print(
    "  \\caption{Adapter performance evaluated on the testing split (\\texttt{ensemble\\_test}), comparing representations formed by concatenated target embeddings with versus without similarity (RDM) features constraint. The ensemble size is $K=10$ and the number of similarity features is $D_{\\text{sim}}=45$.}"
)
print("  \\label{tab:adapter_results}")
print("  \\begin{tabular}{lcccc}")
print("    \\hline")
print(
    "    Ensemble & Mean Comp. & Soft Voting & Acc (Embed Only) & Acc (Embed + Sim) \\\\"
)
print("    \\hline")
...
for r in results:
    ...
    metrics_path = os.path.join(
        get_ensemble_path_by_name(r["name"]), "adapters_metrics.json"
    )
    with open(metrics_path, "r") as f:
        metrics = json.load(f)
        K = metrics.get("M", "?")
    ...
    print(
        f"    \\texttt{{{safe_name}}} & {comp_acc:.2f}\\% & {ens_acc:.2f}\\% & {acc_only:.2f}\\% & {acc_sim:.2f}\\% \\\\"
    )
