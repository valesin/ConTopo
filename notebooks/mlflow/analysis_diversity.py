...
if not div_df.is_empty() and "params.diversity_metric" in div_df.columns:
    print("\nUnique diversity metrics:", sorted(div_df["params.diversity_metric"].drop_nulls().unique().to_list()))
...
if not ens_df.is_empty() and "tags.rho" in ens_df.columns:
    print("Unique rho values:       ", sorted(ens_df["tags.rho"].drop_nulls().unique().to_list()))
...
if merged.is_empty():
    raise RuntimeError("No diversity runs matched the filtered ensembles. Check component_set_hash alignment.")
...
for i, metric in enumerate(available_metrics):
    grp = plot_df.filter(pl.col("params.diversity_metric") == metric).sort("rho_numeric")
    x = [rho_to_pos[v] for v in grp["rho_numeric"].to_list()]
    fig1.add_trace(go.Scatter(
        x=x, y=grp["value"].to_list(),
        name=metric,
        mode="lines+markers",
...
def _metric_label(m):
    arrow = "↑ more diverse" if METRIC_HIGHER_IS_MORE_DIVERSE.get(m, True) else "↓ more diverse"
    return f"{m}  ({arrow})"
...
fig2.update_xaxes(tickvals=list(range(len(rho_vals))), ticktext=rho_labels, title_text="ρ", row=n_metrics, col=1)
fig2.update_layout(
...
fig3 = go.Figure(go.Table(
    ...
    cells=dict(
        values=[
            ...
            ["↑ more diverse" if METRIC_HIGHER_IS_MORE_DIVERSE.get(row["metric"], True) else "↓ more diverse" for row in corr_rows],
