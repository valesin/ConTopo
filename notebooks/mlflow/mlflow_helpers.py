...
def get_consistency_results(split: str = "test") -> pd.DataFrame:
    """Normalised consistency results for the given split.

    Rho is read directly from params.rho. "mixed" values (cross-rho combos) are
    preserved as-is and produce NaN in rho_numeric.

    Columns: run_id, cs_hash, ensemble_name, num_components, mean_rsa_correlation,
             rho, rho_numeric
    """
    runs = get_runs("consistency", split=split)
    if runs.empty:
        return pd.DataFrame()
    ...
    cols = {
        ...
        "params.rho": "rho",
        "metrics.mean_rsa_correlation": "mean_rsa_correlation",
    }
    df = runs.rename(columns=cols)
    result = df[[c for c in cols.values() if c in df.columns]].copy()
    result["rho_numeric"] = pd.to_numeric(result["rho"], errors="coerce")
    return result.sort_values("rho_numeric").reset_index(drop=True)


def get_consistency_results_for_groups(
    groups_name: str,
    split: str = "test",
) -> pd.DataFrame:
    """Normalized consistency results for all runs generated by a named groups config.

    Uses component_set_hash matching — same mechanism as get_ensemble_results_for_groups.
    Columns: same as get_consistency_results().
    """
    hashes = get_expected_component_hashes(groups_name)
    df = get_consistency_results(split=split)
    if df.empty:
        return df
    return df[df["cs_hash"].isin(hashes)].copy()
